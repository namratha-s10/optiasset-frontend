...
export default function Page() {
  ...
  const [userEmail, setUserEmail] = useState("admin@optivault.com")

  useEffect(() => {
    // Sync with AppLayout localStorage
    ...
    async function loadAssets() {
      try {
        const data = await getAssets(savedEmail)
        setAssets(data)
      } catch (err) {
  ...
  const handleRevoke = async (id: number) => {
    if (userRole !== "Admin") {
      alert("Access Denied: Only Admins can delete assets.")
      return
    }

    if (confirm("Are you sure you want to delete this asset?")) {
      try {
        await deleteAsset(id, userEmail)
  ...
  const handleAddAsset = async (e: React.FormEvent) => {
    e.preventDefault()
    if (userRole !== "Admin") {
      alert("Access Denied: Only Admins can register assets.")
      return
    }
    ...
    try {
      setLoading(true)
      const created = await createAsset(newAsset, userEmail)
    ...
    <div className="flex flex-col gap-8 w-full max-w-6xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex justify-between items-center bg-card p-6 rounded-2xl border shadow-sm">
        ...
        <div className="hidden sm:block">
          <Button 
            onClick={() => isAdmin ? setShowAddForm(!showAddForm) : alert("Only Admins can add assets.")}
            className={`font-semibold shadow-md ${!isAdmin ? 'opacity-50 cursor-not-allowed' : ''}`}
                  ...
                  <tr key={asset.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      <Button 
                        variant="outline" 
                        size="sm" 
                        onClick={() => isAdmin ? handleRevoke(asset.id) : alert("Only Admins can delete assets.")}
                        disabled={!isAdmin}
                        className={`h-8 shadow-xs transition-colors ${isAdmin ? 'hover:bg-destructive/10 hover:text-destructive hover:border-destructive' : 'opacity-50 cursor-not-allowed'}`}
                      >
                        Delete
                      </Button>
                    </td>
